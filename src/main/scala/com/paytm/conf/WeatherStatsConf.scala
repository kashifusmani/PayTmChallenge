...
class WeatherStatsConf(arguments: Seq[String]) extends ScallopConf(arguments) {
  val appName: ScallopOption[String] = opt[String](
    required = false,
    default = Some("PayTm Weather Challenge")
  )
  ...
  val cleanedCountriesFileName: ScallopOption[String] = opt[String](
    required = false,
    default = Some("countrylist_cleaned.csv")
  )
